...
public class Listado extends AppCompatActivity implements LifecycleObserver {
    ListView lista;
    ArrayList<RegistroPersonas> listat;
...
ListAdap adapter;
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 2) {
            if (resultCode == Activity.RESULT_OK) {
                ArrayList<RegistroPersonas> listaServis = control.obtenerPersona();
    ...
    private void modificar(int posicion) {
        Intent intent = new Intent(this, ModificarRegistro.class);
